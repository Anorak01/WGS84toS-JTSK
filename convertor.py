...
# Global variables to store correction tables
CORRTABLE = {}
QUASIGEOID = {}
...
def readQuasigeoid():
    ...
    for line in f:
        row = line.split()
        if len(row) == 3:  # Skip header lines
            lat, lon = float(row[0]), float(row[1])
            lat = float(round(lat, 3))
            lon = float(round(lon, 3))
            QUASIGEOID[(lat, lon)] = float(row[2])
    f.close()


def get_quasigeoid_correction(latitude, longitude):
    """
    ...
    try:
        ...
        h00 = QUASIGEOID[(lat0, lon0)]
        h01 = QUASIGEOID[(lat0, lon1)]
        h10 = QUASIGEOID[(lat1, lon0)]
        h11 = QUASIGEOID[(lat1, lon1)]

        # Calculate interpolation weights
        wx = (longitude - lon0) / dlon
        ...
        # If point is outside the grid, return None
        return None
...
def convertToJTSK(latitude, longitude, height=0):
    ...
    if latitude < 40 or latitude > 60 or longitude < 5 or longitude > 25:
        ...
        return [round(-Y, 2), round(-X, 2), round(bpv_height, 3)]
...
def bessel_to_jtsk(B, L, H=0):
    ...
    # latitude scaling - defines the oblique aspect
    ...
    dY = (
        ...
        - 4 * A10 * Yred * Xred * (Xred**2 - Yred**2)
        + A9 * (Xred**4 + Yred**4 - 6 * Xred**2 * Yred**2)
    )
    dX = (
        A1
        + A3 * Xred
        ...
        + 4 * A9 * Yred * Xred * (Xred**2 - Yred**2)
        + A10 * (Xred**4 + Yred**4 - 6 * Xred**2 * Yred**2)
    )
    ...
    # Convert ellipsoidal height to Baltic height system (Bpv)
    # The height remains unchanged in this transformation as it's handled separately
    # The actual height conversion should be done using proper quasi-geoid model (CR2005 or newer)
...
def jtsk_to_bessel(X05, Y05):
    ...
    while True:
        B = 2 * (
            math.atan(
                k ** (-1 / alfa)
                * math.tan(U / 2 + math.radians(45)) ** (1 / alfa)
                * ((1 + e * math.sin(Bi)) / (1 - e * math.sin(Bi))) ** (e / 2)
            )
            - math.radians(45)
        )
        ...
        for i2, parts in enumerate(parts2):
            ...
            for i3, parts in enumerate(parts3):
                ...
                if len(parts) > 0 and contains_two_floats(parts):
                    ...
                    parts3[i3] = f"{x1} {x2}"
            if len(parts3) > 1:
                parts2[i2] = ")".join(parts3)
            else:
                parts2[i2] = parts3[0]
        if len(parts2) > 1:
            spgeom[i1] = "(".join(parts2)
        else:
            spgeom[i1] = parts2[0]
    if len(spgeom) > 1:
        geom = ", ".join(spgeom)
