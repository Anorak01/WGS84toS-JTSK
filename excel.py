...
START_ROW = 3 # 2 in zero index
RECORD_INDEX_COL = 7 # 4 in zero index
LATITUDE_COL = 9 # 6 in zero index
LONGITUDE_COL = 10 # 7 in zero index
HEIGHT_COL = 34
FEATURE_DESCRIPTION_COL = 47 # 44 in zero index
# 45
...
def process_sheet(file_location: String, save_directory: String, negative_xy: bool) -> bool: # returns success
    ...
    with open(save_directory + "/" + file_name + ".txt", "w", encoding="utf-8") as f:
        #f.write("# Číslo bodu     Y     X     Z    Popis bodu\n")
...
#        for record in records:
 #           f.write(f"  {str(record.index).ljust(index_max_len, " ")} {str(record.latitude).ljust(latitude_max_len, " ")} {str(record.longitude).ljust(longitude_max_len, " ")} 0 {record.description}\n")
